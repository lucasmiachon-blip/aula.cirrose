...
if [ -f "$TEMPLATE" ]; then
  ...
  EXPECTED_ORDER="./shared/css/base.css
./archetypes.css
./cirrose.css"

  if [ "$IMPORTS" = "$EXPECTED_ORDER" ]; then
    echo "  PASS: base -> archetypes -> cirrose"
  else
    echo "  FAIL: Import order is wrong!"
    echo "  Expected: base -> archetypes -> cirrose"
    echo "  Got:"
    echo "$IMPORTS" | sed 's/^/    /'
    FAIL=$((FAIL + 1))
  fi
...
CSS_FILES=()
for f in "$DIR/shared/css/base.css" "$DIR/archetypes.css" "$DIR/cirrose.css" "$DIR/$AULA.css"; do
  [ -f "$f" ] && CSS_FILES+=("$f")
done
